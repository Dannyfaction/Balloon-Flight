...
namespace SplineTool
{
    ...
    public class Spline : ScriptableObject, System.Collections.IEnumerable
    {
        ...
        public void Subdivide(int i1, int i2)
        {
            ...
            Insert(ref waypoints, Length, new Waypoint(center));
            Waypoint temp = new Waypoint(center);
            Insert(ref temp.subways, temp.subways.Length, i2);
            Insert(ref temp.reversedWays, temp.reversedWays.Length, i1);
            Insert(ref temp.handles, 0, temp.position + (s.position - temp.position).normalized * handleOffest);
            Insert(ref temp.handles, temp.handles.Length, temp.position + (f.position - temp.position).normalized * handleOffest);
        ...
        {
            int sw = IndexOf(waypoints, p2);
            sw = IndexOf(p1.subways, sw);
            int rw = IndexOf(waypoints, p1);
            rw = IndexOf(p2.reversedWays, rw);

            Remove(ref p1.handles, sw);
            Remove(ref p1.subways, sw);
            Remove(ref p2.handles, p2.GetHandleFromReverseWays(rw));
            Remove(ref p2.reversedWays, rw);
        }
        ...
        void ConnectSubway(int i1, int i2)
        {
            ...
            Insert(ref p1.subways, p1.subways.Length, i2);
            Insert(ref p1.handles, p1.subways.Length - 1, p1.position + (waypoints[i2].position - p1.position).normalized * handleOffest);
            Insert(ref p2.reversedWays, p2.reversedWays.Length, i1);
            Insert(ref p2.handles, p2.handles.Length, p2.position + (waypoints[i1].position - p2.position).normalized * handleOffest);
        }

        void ConnectSubway(Waypoint p1, Waypoint p2)
        {
            int i1 = IndexOf(waypoints, p1);
            int i2 = IndexOf(waypoints, p2);
            Insert(ref p1.subways, p1.subways.Length, i2);
            Insert(ref p1.handles, p1.subways.Length - 1, p1.position + (waypoints[i2].position - p1.position).normalized * handleOffest);
            Insert(ref p2.reversedWays, p2.reversedWays.Length, i1);
            Insert(ref p2.handles, p2.handles.Length, p2.position + (waypoints[i1].position - p2.position).normalized * handleOffest);
        ...
        /// <summary>
        /// Make the point a sharp edge by adjusting the handles
        /// </summary>
        public void Sharpen(int i)
        {
            for (int s = 0; s < waypoints[i].subways.Length; s++)
            {
                waypoints[i].handles[s] = waypoints[i].position + (waypoints[waypoints[i].subways[s]].position - waypoints[i].position).normalized;
            }
            for (int s = 0; s < waypoints[i].reversedWays.Length; s++)
            {
                waypoints[i].handles[waypoints[i].GetHandleFromReverseWays(s)] = waypoints[i].position + (waypoints[waypoints[i].reversedWays[s]].position - waypoints[i].position).normalized;
            }
        }
